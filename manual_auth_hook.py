...
def create_dns_record(domain_zone, wildcard_domain, validation_token):
    ...
    regru_api_url = "https://api.reg.ru/api/regru2/zone/add_txt"
    post_data = {
        "input_format": "json",
    ...
    try:
        ...
        log("error", f"Failed to create DNS record: {e}")
        sys.exit(1)
    ...
    log("success", f"DNS-TXT record was created on Reg.ru for subdomain: {subdomain}")
    log("info", f"Waiting for the txt record to become available on Claudflare's DNS servers")
    log("info", f"Parameters: max_attempts={MAX_ATTEMPTS}, delay={DELAY}s")

    attempt_counter = 0
    while attempt_counter < MAX_ATTEMPTS:
        result = subprocess.run(
            ["dig", "-t", "txt", f"{subdomain}.{domain_zone}", "@1.1.1.1", "+short"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        if validation_token in result.stdout.decode():
            log("success", f"TXT-record found after ~{(attempt_counter * DELAY) / 60}m.")
            return

        log("info", f"({attempt_counter+1}/{MAX_ATTEMPTS}) The TXT-record is not yet available")
        attempt_counter += 1
        time.sleep(DELAY)
...
if __name__ == "__main__":
    ...
    create_dns_record(domain_zone, wildcard_domain, token)